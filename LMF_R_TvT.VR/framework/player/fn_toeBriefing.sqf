...

{
	...
	//UPDATE OLD
	_grp2 = _grp;
} foreach ((playableUnits + switchableUnits) select {side _x == _side});
...
%2<br/>
",count ((switchableUnits + playableUnits - entities 'HeadlessClient_F') select {side _x == _side}),_brf_platoon];
